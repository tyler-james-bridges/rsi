...
function serialize(value: unknown, ancestors: WeakSet<object>, depth: number): string {
  ...
  if (typeof value === "number") {
    if (!Number.isSafeInteger(value))
      fail("INPUT_INVALID", "Canonical numbers must be safe integers");
    return String(value);
  }
...
export function exactArray(value: unknown, safeName: string): readonly unknown[] {
  ...
  if (keys.length !== allowed.size || keys.some((key) => !allowed.has(key as string))) {
...
export function copyBytes(value: unknown, safeName: string, maximumBytes = 16 * 1024): Buffer {
  if (utilTypes.isProxy(value) || !utilTypes.isUint8Array(value)) {
    fail("INPUT_INVALID", `${safeName} must be a byte array`);
  }
  const bytes = value as Uint8Array;
  if (utilTypes.isSharedArrayBuffer(bytes.buffer)) {
    fail("INPUT_INVALID", `${safeName} may not use shared memory`);
  }
...
export function validateCommit(value: unknown): string {
  if (typeof value !== "string" || !COMMIT_PATTERN.test(value)) {
    fail("INPUT_INVALID", "Release commit must be a lowercase 40-character digest");
  }
  return value;
}
...
export function validateSafeInteger(value: unknown, safeName: string, minimum: number): number {
...
export function validateSemver(value: unknown): string {
  if (
    typeof value !== "string" ||
    value.length > 128 ||
    ...
    fail("INPUT_INVALID", "Release version must be canonical SemVer");
  }
  return value;
}
...
export function decodeCanonicalBase64url(
  ...
  if (typeof value !== "string" || !/^[A-Za-z0-9_-]+$/.test(value)) {
    fail("INPUT_INVALID", `${safeName} encoding is invalid`);
  }
  const bytes = Buffer.from(value, "base64url");
